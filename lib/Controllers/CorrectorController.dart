...
String aiGuidlines = ''' You are a grammar corrector. Below is the provided text, follow these steps:

1. Analyze the text for grammatical errors.
2. Important: Highlight each mistake by enclosing the words or phrases in large brackets [].
3. After highlighting the mistakes, provide a corrected version of the text.
4. Important: Highlight each corrected word by enclosing the words or phrases in curly brackets {}.
5. Ensure the corrected text is grammatically correct, clear, and well-structured.
6. Note: If the text contains a person/human name, then leave the name as it is.
7. Extra notes: 
   ...
   - Don't use complicated words that are difficult to understand.
...
Provided text:"John and me goes to the store. He dont like the store's layout, it was too cluttered. Their were many products but not much organization. I seen a lot of items that catch my eye. We buyed some fruits, vegetables, and breads. On our way back, John sayed he forget his wallet at home. We was worried about how to pay, but I remember I have some cash with me."
...
Here is the text in which you have to apply these rules:
...
Future<String> sendQuery(BuildContext context) async {
  ...
  try {
    ...
    if (_isValidJson(res)) {
      ...
      outputText.value =
          removeBracketsAndSetOutput(correctedText.value);
      ...
      if (outputText.value.isNotEmpty) {
        isresultLoaded.value = true;
         ...
         Get.to(()=>CorrectorDetails(mistaletext: highlightedMistakes.value , correctedText: correctedText.value,));
      }
    } else {
  ...
  text.splitMapJoin(
    ...
    onMatch: (Match match) {
      ...
      if (matchText.startsWith('[') && matchText.endsWith(']')) {
        children.add(
          TextSpan(
            text: matchText.substring(1, matchText.length - 1), // Remove brackets
